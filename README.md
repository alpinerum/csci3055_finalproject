...
An if statement is done in this form,
```
if (expression) {
  statement(s)
}
```
an if-else statement is in this form
```
if (expression) {
  statement(s)
}
...
```
where the statements in else are executed if the expression is not true. An if-else-if statement is done in this form,
```
if (expression) {
  statement(s)
}
